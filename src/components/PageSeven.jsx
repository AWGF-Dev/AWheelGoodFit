...
const PageSeven = ({ title, activeSubtab, onSubtabClick }) => {
  const subtabs = [
    {
      id: "intro",
      label: "stink1",
  ...
  const handlePrev = () => {
    if (currentIndex > 0) onSubtabClick(subtabs[currentIndex - 1].id, "page7");
  };

  const handleNext = () => {
    if (currentIndex < subtabs.length - 1) onSubtabClick(subtabs[currentIndex + 1].id, "page7");
  };
      ...
      <div className="w-72 bg-gray-100 border-r border-gray-300 p-6 flex-shrink-0 h-screen flex flex-col transition-opacity duration-300">
        ...
        <div className="flex flex-col gap-3">
          {subtabs.map(tab => (
            <div
              key={tab.id}
              onClick={() => onSubtabClick(tab.id, "page7")}

...
const PageSix = ({ title, activeSubtab, onSubtabClick }) => {
  const subtabs = [
    {
      id: "intro",
      label: "Pelvis",
  ...
  const handlePrev = () => {
    if (currentIndex > 0) onSubtabClick(subtabs[currentIndex - 1].id, "page6");
  };

  const handleNext = () => {
    if (currentIndex < subtabs.length - 1) onSubtabClick(subtabs[currentIndex + 1].id, "page6");
  };
  ...
  return (
    <div className="w-full h-full flex">
      ...
      <div className="w-72 bg-gray-100 border-r border-gray-300 p-6 flex-shrink-0 h-screen flex flex-col transition-opacity duration-300">
        <p className="text-3xl font-mono font-bold mb-6">{title}</p>
        <div className="flex flex-col gap-3">
          {subtabs.map(tab => (
            <div
              key={tab.id}
              onClick={() => onSubtabClick(tab.id, "page6")}
